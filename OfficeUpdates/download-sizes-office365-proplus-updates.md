---
title: Office 365 專業增強版更新的下載大小
ms.author: andrewmo
author: andymosten
manager: andrewmo
ms.date: 9/6/2018
ms.audience: ITPro
ms.topic: reference
ms.service: o365-proplus-itpro
localization_priority: Priority
ms.collection: RelNotes_ProPlus
description: 為 IT 專業人員提供 Office 365 專業增強版每個更新通道的更新大約下載大小相關資訊
ms.openlocfilehash: dcd67d8313f05a51c041799cd3a3c642334e98d8
ms.sourcegitcommit: 54d6714d1475760fbdc61dff43d252883dfbf505
ms.translationtype: HT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/06/2018
ms.locfileid: "23848381"
---
# <a name="download-sizes-for-updates-to-office-365-proplus"></a>Office 365 專業增強版更新的下載大小

下表顯示 (依發行日期和更新通道) 從舊版 Office 365 專業增強版更新至最新版本時，大約的下載大小。 
  
我們在實際發行日期之前提供此資訊，以協助企業客戶為這些即將推出的通道更新計劃網路頻寬使用率。因為我們預先提供此資訊，某些資訊可能會在實際發佈時變更。
  
我們的目標是要在發行前一週提供更新大小資訊，但有時這項資訊無法事先取得。半年通道和半年通道 (已設定目標) 預計會在每個月的第二個星期二發行。每月通道則沒有一定的發行排程。

如需指定版本中所含功能、安全性更新和非安全性更新的相關資訊，請參閱 [Office 365 專業增強版更新的版本資訊](release-notes-office365-proplus.md)。

  
> [!NOTE]
> - 表格中所列的大約下載大小僅適用於 Office 365 專業增強版的 x86 英文版。 
> - 下載大小是近似值。實際下載大小可能有所不同，可能多達 50 MB。 
> - 發行日期與組建編號 (斜體字並以 "\< \>" 括住) 可能會變更。 
> - 只會列出最近四個月的版本。
> - 只有最近使用的舊版本會列示於表格中。如果您正在從舊版進行更新，下載大小就會變大。
> - 如果您正在切換至不同的通道，例如從「半年通道 (已設定目標) 」到「半年通道」，更新大小會遠超過表格中所列的更新大小。它可能需要完整下載，下載大小至少 1 GB。 

  ## <a name="september-5-2018"></a>2018 年 9 月 5 日

### <a name="monthly-channel"></a>每月頻道

最新版本是 1808 版 (組建 10730.20088)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1807 (組建 10325.20118)  <br/> |2018 年 8 月 14 日  <br/> |231  <br/> |
|版本 1807 (組建 10325.20082)  <br/> |2018 年 7 月 25 日  <br/> |237  <br/> |

  ## <a name="august-14-2018"></a>2018 年 8 月 14 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1807 版 (組建 10325.20118)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1807 (組建 10325.20082)  <br/> |2018 年 7 月 25 日  <br/> |82  <br/> |
|版本 1807 (組建 10228.20134)  <br/> |2018 年 7 月 17 日  <br/> |183  <br/> |

  
### <a name="semi-annual-channel"></a>半年通道

最新版本是 1803 版 (組建 9126.2275)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1803 (組建 8431.2259)  <br/> |2018 年 7 月 10 日  <br/> |86  <br/> |
|版本 1708 (組建 8431.2270)  <br/> |2018 年 6 月 12 日  <br/> |279  <br/> |

  
### <a name="semi-annual-channel-targeted"></a>半年頻道 (已設定目標)

最新版本是 1803 版 (組建 9126.2275)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1803 (組建 8431.2259)  <br/> |2018 年 7 月 10 日  <br/> |86  <br/> |
|版本 1803 (組建 9126.2227)  <br/> |2018 年 6 月 12 日  <br/> |94  <br/> |

  ## <a name="july-25-2018"></a>2018 年 7 月 25 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1807 版 (組建 10325.20082)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1806 (組建 10228.20314)  <br/> |2018 年 7 月 17 日  <br/> |176  <br/> |
|版本 1805 (組建 10228.20104)  <br/> |2018 年 7 月 10 日  <br/> |176  <br/> |

 ## <a name="july-10-2018"></a>2018 年 7 月 10 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1806 版 (組建 10228.20104)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1805 (組建 10228.20080)  <br/> |2018 年 6 月 25 日  <br/> |79  <br/> |
|版本 1805 (組建 9330.2124)  <br/> |2018 年 6 月 13 日  <br/> |149  <br/> |
|版本 1805 (組建 9330.2118)  <br/> |2018 年 6 月 12 日  <br/> |401  <br/> |
|版本 1805 (組建 9330.2087)  <br/> |2018 年 5 月 24 日  <br/> |150  <br/> |

最新版本是 1803 版 (組建 9126.2259)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1708 (組建 8431.2270)  <br/> |2018 年 6 月 12 日  <br/> |280  <br/> |
|版本 1708 (組建 8431.2250)  <br/> |2018 年 5 月 8 日  <br/> |280  <br/> |
   
### <a name="semi-annual-channel-targeted"></a>半年頻道 (已設定目標)

最新版本是 1803 版 (組建 9126.2259)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1803 (組建 9126.2227)  <br/> |2018 年 6 月 12 日  <br/> |78  <br/> |
|版本 1803 (組建 9126.2210)  <br/> |2018 年 5 月 18 日  <br/> |85  <br/> | 


## <a name="june-25-2018"></a>2018 年 6 月 25 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是版本 1806 (組建 10228.20080)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1805 (組建 9330.2124)  <br/> |2018 年 6 月 13 日  <br/> |155  <br/> |
|版本 1805 (組建 9330.2118)  <br/> |2018 年 6 月 12 日  <br/> |402  <br/> |
|版本 1805 (組建 9330.2087)  <br/> |2018 年 5 月 24 日  <br/> |154  <br/> |
   
## <a name="june-13-2018"></a>2018 年 6 月 13 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1805 版 (組建 9330.2118)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1805 (組建 9330.2118)  <br/> |2018 年 6 月 12 日  <br/> |57  <br/> |
|版本 1805 (組建 9330.2087)  <br/> |2018 年 5 月 24 日  <br/> |83  <br/> |
|版本 1805 (組建 9330.2078)  <br/> |2018 年 5 月 23 日  <br/> |262  <br/> |
|版本 1804 (組建 9226.2156)  <br/> |2018 年 5 月 14 日  <br/> |203  <br/> |
   
## <a name="june-12-2018"></a>2018 年 6 月 12 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1805 版 (組建 9330.2118)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1805 (組建 9330.2087)  <br/> |2018 年 5 月 24 日  <br/> |95  <br/> |
|版本 1805 (組建 9330.2078)  <br/> |2018 年 5 月 23 日  <br/> |262  <br/> |
|版本 1804 (組建 9226.2156)  <br/> |2018 年 5 月 14 日  <br/> |203  <br/> |
   
### <a name="semi-annual-channel"></a>半年通道

最新的版本是版本 1708 (組建 8431.2270)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1708 (組建 8431.2250)  <br/> |2018 年 5 月 8 日  <br/> |56  <br/> |
|版本 1708 (組建 8431.2242)  <br/> |2018 年 4 月 10 日  <br/> |57  <br/> |
   
### <a name="semi-annual-channel-targeted"></a>半年頻道 (已設定目標)

最新版本是 1803 版 (組建 9126.2275)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1708 (組建 8431.2227)  <br/> |2018 年 6 月 12 日  <br/> |94  <br/> |
|版本 1708 (組建 8431.2259)  <br/> |2018 年 7 月 10 日  <br/> |86  <br/> |
|版本 1708 (組建 8431.2270)  <br/> |2018 年 6 月 12 日  <br/> |279  <br/> |

最新的版本是版本 1803 (組建 9126.2227)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1803 (組建 9126.2210)  <br/> |2018 年 5 月 18 日  <br/> |94  <br/> |
|版本 1803 (組建 9126.2191)  <br/> |2018 年 5 月 8 日  <br/> |96  <br/> |



## <a name="may-24-2018"></a>2018 年 5 月 24 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1805 版 (組建 9330.2087)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1805 (組建 9330.2078)  <br/> |2018 年 5 月 23 日  <br/> |76  <br/> |
|版本 1804 (組建 9226.2156)  <br/> |2018 年 5 月 14 日  <br/> |203  <br/> |
|版本 1804 (組建 9226.2126)  <br/> |2018 年 5 月 8 日  <br/> |203  <br/> |



## <a name="may-23-2018"></a>2018 年 5 月 23 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1805 版 (組建 9330.2078)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1804 (組建 9226.2156)  <br/> |2018 年 5 月 14 日  <br/> |203  <br/> |
|版本 1804 (組建 9226.2126)  <br/> |2018 年 5 月 8 日  <br/> |203  <br/> |



## <a name="may-18-2018"></a>2018 年 5 月 18 日

### <a name="semi-annual-channel-targeted"></a>半年頻道 (已設定目標)

最新的版本是 1803 版 (組建 9126.2210)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1803 (組建 9126.2191)  <br/> |2018 年 5 月 8 日  <br/> |62  <br/> |
|版本 1803 (組建 9126.2152)  <br/> |2018 年 4 月 10 日  <br/> |91  <br/> |

 
## <a name="may-14-2018"></a>2018 年 5 月 14 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1804 版 (組建 9226.2156)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1804 (組建 9226.2126)  <br/> |2018 年 5 月 8 日  <br/> |67  <br/> |
|版本 1804 (組建 9226.2114)  <br/> |2018 年 4 月 25 日  <br/> |81  <br/> |


## <a name="may-8-2018"></a>2018 年 5 月 8 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1804 版 (組建 9226.2126)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1804 (組建 9226.2114)  <br/> |2018 年 4 月 25 日  <br/> |80  <br/> |
|版本 1803 (組建 9126.2152)  <br/> |2018 年 4 月 11 日  <br/> |162  <br/> |
|版本 1803 (組建 9126.2116)  <br/> |2018 年 3 月 27 日  <br/> |162  <br/> |
   
### <a name="semi-annual-channel"></a>半年通道

最新的版本是 1708 版 (組建 8431.2250)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
| 版本 1708 (組建 8431.2242)  <br/> |2018 年 4 月 10 日  <br/> |49  <br/> |
|版本 1708 (組建 8431.2236)  <br/> |2018 年 3 月 13 日  <br/> |53  <br/> |
   
### <a name="semi-annual-channel-targeted"></a>半年頻道 (已設定目標)

最新的版本是 1803 版 (組建 9126.2191)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1803 (組建 9126.2152)  <br/> |2018 年 4 月 10 日  <br/> |90  <br/> |
|版本 1803 (組建 9126.2098)  <br/> |2018 年 3 月 20 日  <br/> |114  <br/> |
   
## <a name="april-25-2018"></a>2018 年 4 月 25 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1804 版 (組建 9226.2114)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1803 (組建 9126.2152)  <br/> |2018 年 4 月 11 日  <br/> |161  <br/> |
|版本 1803 (組建 9126.2116)  <br/> |2018 年 3 月 27 日  <br/> |161  <br/> |
   
## <a name="april-11-2018"></a>2018 年 4 月 11 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1803 版 (組建 9126.2152)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1803 (組建 9126.2116)  <br/> |2018 年 3 月 27 日  <br/> |102  <br/> |
|版本 1802 (組建 9029.2253)  <br/> |2018 年 3 月 13 日  <br/> |186  <br/> |
   
## <a name="april-10-2018"></a>2018 年 4 月 10 日

### <a name="semi-annual-channel"></a>半年通道

最新的版本是 1708 版 (組建 8431.2242)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1708 (組建 8431.2236)  <br/> |2018 年 3 月 13 日  <br/> |54  <br/> |
|版本 1708 (組建 8431.2215)  <br/> |2018 年 2 月 13 日  <br/> |54  <br/> |
   
### <a name="semi-annual-channel-targeted"></a>半年頻道 (已設定目標)

最新的版本是 1803 版 (組建 9126.2152)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1803 (組建 9126.2098)  <br/> |2018 年 3 月 20 日  <br/> |111  <br/> |
|版本 1803 (組建 9126.2072)  <br/> |2018 年 3 月 13 日  <br/> |113  <br/> |
   
## <a name="march-27-2018"></a>2018 年 3 月 27 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1803 版 (組建 9126.2116)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1802 (組建 9029.2253)  <br/> |2018 年 3 月 13 日  <br/> |186  <br/> |
|版本 1802 (組建 9029.2167)  <br/> |2018 年 2 月 26 日  <br/> |185  <br/> |
   
## <a name="march-20-2018"></a>2018 年 3 月 20 日

### <a name="semi-annual-channel-targeted"></a>半年頻道 (已設定目標)

最新的版本是 1803 版 (組建 9126.2098)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1803 (組建 9126.2072)  <br/> |2018 年 3 月 13 日  <br/> |92  <br/> |
|版本 1708 (組建 8431.2215)  <br/> |2018 年 2 月 13 日  <br/> |309  <br/> |
   
## <a name="march-13-2018"></a>2018 年 3 月 13 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1802 版 (組建 9029.2253)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1802 (組建 9029.2167)  <br/> |2018 年 2 月 26 日  <br/> |98  <br/> |
|版本 1801 (組建 9001.2171)  <br/> |2018 年 2 月 13 日  <br/> |183  <br/> |
   
### <a name="semi-annual-channel"></a>半年通道

最新的版本是 1708 版 (組建 8431.2236)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1708 (組建 8431.2215)  <br/> |2018 年 2 月 13 日  <br/> |50  <br/> |
|版本 1708 (組建 8431.2153)  <br/> |2018 年 1 月 9 日  <br/> |54  <br/> |
   
### <a name="semi-annual-channel-targeted"></a>半年頻道 (已設定目標)

最新的版本是 1803 版 (組建 9126.2072)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1708 (組建 8431.2215)  <br/> |2018 年 2 月 13 日  <br/> |320  <br/> |
|版本 1708 (組建 8431.2153)  <br/> |2018 年 1 月 9 日  <br/> |309  <br/> |
   
## <a name="february-26-2018"></a>2018 年 2 月 26 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1802 版 (組建 9029.2167)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1801 (組建 9001.2171)  <br/> |2018 年 2 月 13 日  <br/> |189  <br/> |
|版本 1801 (組建 9001.2144)  <br/> |2018 年 2 月 7 日  <br/> |329  <br/> |
|版本 1801 (組建 9001.2138)  <br/> |2018 年 2 月 1 日  <br/> |193  <br/> |
   
## <a name="february-13-2018"></a>2018 年 2 月 13 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1801 版 (組建 9001.2171)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1801 (組建 9001.2144)  <br/> |2018 年 2 月 7 日  <br/> |80  <br/> |
|版本 1801 (組建 9001.2138)  <br/> |2018 年 2 月 1 日  <br/> |83  <br/> |
|版本 1712 (組建 8827.2179)  <br/> |2018 年 1 月 30 日  <br/> |358  <br/> |
|版本 1712 (組建 8827.2148)  <br/> |2018 年 1 月 17 日  <br/> |174  <br/> |
   
### <a name="semi-annual-channel"></a>半年通道

最新的版本是 1708 版 (組建 8431.2215)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1708 (組建 8431.2153)  <br/> |2018 年 1 月 9 日  <br/> |53  <br/> |
|版本 1705 (組建 8201.2213)  <br/> |2017 年 12 月 12 日  <br/> |293  <br/> |
   
### <a name="semi-annual-channel-targeted"></a>半年頻道 (已設定目標)

最新的版本是 1708 版 (組建 8431.2215)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1708 (組建 8431.2153)  <br/> |2018 年 1 月 9 日  <br/> |53  <br/> |
|版本 1708 (組建 8431.2131)  <br/> |2017 年 12 月 12 日  <br/> |62  <br/> |
   
## <a name="february-7-2018"></a>2018 年 2 月 7 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1801 版 (組建 9001.2144)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1801 (組建 9001.2138)  <br/> |2018 年 2 月 1 日  <br/> |57  <br/> |
|版本 1712 (組建 8827.2179)  <br/> |2018 年 1 月 30 日  <br/> |174  <br/> |
|版本 1712 (組建 8827.2148)  <br/> |2018 年 1 月 17 日  <br/> |215  <br/> |
|版本 1711 (組建 8730.2175)  <br/> |2018 年 1 月 9 日  <br/> |233  <br/> |
   
## <a name="february-1-2018"></a>2018 年 2 月 1 日

### <a name="monthly-channel"></a>每月頻道

最新的版本是 1801 版 (組建 9001.2138)。
  
|**升級前的版本**|**發行日期**|**大約下載大小 (以 MB 為單位)**|
|:-----|:-----|:-----|
|版本 1712 (組建 8827.2179)  <br/> |2018 年 1 月 30 日  <br/> |173  <br/> |
|版本 1712 (組建 8827.2148)  <br/> |2018 年 1 月 17 日  <br/> |213  <br/> |
|版本 1711 (組建 8730.2175)  <br/> |2018 年 1 月 9 日  <br/> |233  <br/> |
   
  
## <a name="related-topics"></a>相關主題

- [Office 365 專業增強版更新通道的概觀](https://docs.microsoft.com/DeployOffice/overview-of-update-channels-for-office-365-proplus)
- [Office 365 專業增強版的更新歷程記錄 (依日期列出)](update-history-office365-proplus-by-date.md)
- [Office 365 專業增強版的更新歷程記錄 (依版本列出)](update-history-office365-proplus-by-version.md)

> [!NOTE]
> 如果您在使用 Office 時需要幫忙處理問題的話，我們建議您張貼您的問題在 [Microsoft Answers 論壇 ](https://answers.microsoft.com/) 或 [Tech 社群](https://techcommunity.microsoft.com/)，或者您也可以連絡 [支援中心](https://support.microsoft.com/contactus)。
