...
> [!NOTE]
> - 下面也會提供 Visio Pro for Office 365 和 Project Online Desktop Client 新功能、安全性更新和非安全性更新的相關資訊。
> - 此資訊也適用於 Office 365 商務版。Office 365 商務版是隨部分 Office 365 方案提供的 Office 版本，如商務進階版。
...
*這是從 2018 年 1 月以來所提供的半年通道版本。在 2019 年 3 月之前，我們會繼續支援這個版本並提供安全性更新。不過，我們目前已提供新的半年通道版本：版本　1803 (組建 16.0.9126.2282)，其中包含新功能、安全性更新和非安全性更新。*
...
-   [CVE-2018-8244](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-8244)：Microsoft Outlook 提高權限漏洞
...
-   修正當使用者撰寫電子郵件時，解析顯示名稱會導致 Outlook 當機的問題。
-   修正部分使用者未收到其租用戶系統管理員已啟用之支援功能的問題。
-   修正當搜尋範圍限定為「所有信箱」 ，時搜索失敗並顯示 [找不到相符內容] 的問題。
...
### <a name="powerpoint-non-security-updates"></a>PowerPoint：非安全性更新
-   修正此問題：表格以粗框線錯誤編譯。
-   修正此問題：當變更 Shape.Visibile 屬性時，可能會造成當機。
-   修正此問題：共同撰寫文件的變更無法合併。
...
-   修正與 TLS 1.2 支援相關的問題。
-   修正在會議中選取 [Skype 通話] 新增使用者會產生錯誤的問題。
-   修正在會議中選取 [Skype 通話] 新增使用者會產生錯誤的問題。
...
-   **新的筆跡效果：** 使用金屬畫筆和筆跡效果 (如彩虹、星空、熔岩、海洋、黃金、白銀等) 來展現您的想法。
-   **共用檔案 UI：** 對於商務用 OneDrive 或 SharePoint 檔案，按一下功能區右上角的 [共用] 按鈕，或移至 [檔案] \> [共用] 即可啟動經簡化並改良過的 [共用] 對話方塊。對於新的或儲存在本機的檔案，該 UI 可讓使用者輕鬆地將檔案上傳至 OneDrive 以啟動共同作業。
-   **封鎖危險的副檔名：** 根據預設，具有高風險以及內嵌為 OLE 封裝物件的副檔名 (例如.exe、.vbs 和 .js)，將會遭到封鎖而無法啟動。[深入了解](https://support.office.com/article/packager-activation-in-office-365-desktop-applications-52808039-4a7c-4550-be3a-869dd338d834)
...
### <a name="office-suite-non-security-updates"></a>Office 套件：非安全性更新
-   修正當使用「System Center Configuration Manager」部署更新至執行 Office 應用程式的用戶端時，更新未在 Office 應用程式執行中且重新啟動裝置後套用的問題。
...
-   修正此問題：SVG 測試失敗
-   修正當使用 System Center Configuration Manager 將更新部署至執行 Office 應用程式的用戶端時，更新未在 Office 應用程式執行中且重新啟動裝置後套用的問題。
...
### <a name="outlook-non-security-updates"></a>Outlook：非安全性更新
-   修正導致 Windows 7 SP1 使用者無法將成員新增至「新式群組」的問題。
...
*這是從 2017 年 9 月以來所提供的順延通道版本。在 2018 年 7 月之前，我們會繼續支援這個版本並提供安全性更新。不過，我們目前已提供新的半年通道版本：版本 1708 (組建 8431.2242)，其中包含新功能、安全性更新和非安全性更新。*
...
*這是從 2017 年 9 月以來所提供的順延通道版本。在 2018 年 7 月之前，我們會繼續支援這個版本並提供安全性更新。不過，我們目前已提供新的半年通道版本：版本 1708 (組建 8431.2236)，其中包含新功能、安全性更新和非安全性更新。*
...
*這是從 2017 年 9 月以來所提供的順延通道版本。在 2018 年 7 月之前，我們會繼續支援這個版本並提供安全性更新。不過，我們目前已提供新的半年通道版本：版本 1708 (組建 8431.2215)，其中包含新功能、安全性更新和非安全性更新。*
...
-   **「標籤名稱」屬性：** 藉由建立標籤與表單上控制項的關聯來增強協助工具。
-   **編輯新項目變得更無障礙：** 您可以使用快速鍵盤快速鍵 (Ctrl + E)，從下拉式方塊或清單方塊編輯新項目。
-   **動態連接器：** 從儲存在Microsoft Dynamics 中的資料匯入或連結。[詳細資訊](https://support.office.com/article/636079c1-9fc3-4fca-8410-6596d62223da)
-   **Salesforce 連接器：** 從儲存在 Salesforce 的資料匯入或連結。[詳細資訊](https://support.office.com/article/7375ffb6-1d6a-46f1-bb0c-c6ac3c58f5a0)

### <a name="access-non-security-updates"></a>Access：非安全性更新
-   修正嘗試在文字方塊或下拉式方塊中選取文字時，會選取所有文字，而不是表示選取範圍的問題。
-   修正當查詢與「Microsoft Dynamics」連結的資料表中的主索引鍵有聯結時，查詢不會執行的問題。
-   修正「Microsoft Dynamics」資料表中的貨幣值未顯示小數位數的問題。
...
-   **共用查詢**：將任何查詢定義匯出至 Office 資料庫連線 (ODC) 檔，並在活頁簿之間或與其他人共用。
-   **載入資料：** 從查詢直接將資料載入到樞紐分析表或樞紐分析圖中，而不必將資料儲存到「資料模型」內。
...
-   **增強的資料匯入功能︰** 輕鬆地從各種來源匯入及整理資料。使用「查詢和連線」側邊窗格來管理活頁簿查詢和連線，並透過 ODC 檔與他人共用查詢。[詳細資訊](https://support.office.com/article/ad78befd-eb1c-4ea7-a55d-79d1d67cf9b3)
-   **共用檔案中的變更**：檢視誰在共用活頁簿中進行了變更，以及將活頁簿還原為較舊的版本。[詳細資訊](https://support.office.com/article/5c1e076f-a9c9-41b8-8ace-f77b9642e2c2)
-   **具有畫筆按鈕的「套索選取」：** 使用「套索選取」筆跡支援的數位畫筆按鈕，而不造訪功能區。
...
-   [CVE-2017-11877](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2017-11877)：Microsoft Excel 安全性功能旁路弱點
-   [CVE-2017-11878](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2017-11878)：Microsoft Excel 記憶體損毀弱點
-   [CVE-2017-11884](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2017-11884)：Microsoft Office 記憶體損毀弱點
