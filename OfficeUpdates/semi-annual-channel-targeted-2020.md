...
- **連絡客戶支援視窗現在可停駐：** 尋找 [連絡客戶支援] 視窗目前可停駐在畫面的右窗格。
...
- **在印出的講義上看到投影片編號：** 將投影片編號新增至講義的列印複本。
...
- **更安全的影片體驗：** 安全性增強表示您能以更安全的方式觀看線上影片。
...
- **Microsoft 方程式編輯器轉換程式：** 此轉換程式可讓使用者將使用 Microsoft 方程式編輯器建立的方程式轉換為 Office 數學 ML 格式，以啟用編輯。
