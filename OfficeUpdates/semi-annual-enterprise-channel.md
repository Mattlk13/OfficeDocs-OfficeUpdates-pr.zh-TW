...
- 已修正在商務用 Skype 訊息中複製/貼上時，會導致出現對話方塊顯示「貼上您的內容時發生問題」的問題。
...
- 已修正使用保留來源格式設定選項複製及貼上投影片時，有時會將此設定意外地複製到新的投影片母片的問題
...
- 已修正無法在 Office 中成功開啟檔案的問題。


- 已修正使用複製格式將包含草繪外框的文件套用至連接器可能會損毀的問題。
...
- 已修正使用 [縮放] 視窗時應用程式意外關閉的問題。


- 此修正會解決嘗試執行之前會產生「查詢太複雜」錯誤訊息的特定查詢的問題。


- 此變更可修正當啟動縮放控制方塊 (Shift + F2) 以編輯文字時，可能會導致 Access 意外關閉的問題。
...
- 已修正問題；您應該可以呼叫日期/時間延伸資料類型至您的程式碼，而不會在您的應用程式中遇到任何問題。
...
- 修正若您已從帳戶登出，嘗試建立資料連線時，可能會發生的問題。


- 修正 PivotDateFilter API 中的錯誤，其中的 'Before' 和 'After' 篩選條件是相反的。


- 解決了嘗試將樞紐分析表插入圖表時 Excel 可能會意外關閉的問題。


- 已修正從 Project 計劃匯出時無法編輯樞紐分析表和無法儲存活頁簿的問題。
...
- 已修正在 PowerPivot 中封鎖從 [資料表預覽] 和 [査詢編輯器] 切換的問題。


- 我們已修正使用者無法直接從 SharePoint 開啟 atomsvc (UTF8+BOM) 檔案的問題。


- 已修正客戶在共同撰寫時會錯誤地得到有關文件新版本通知的問題。


- 已修正即使用 IME 和覆寫模式將錯誤地推進額外的字元的編輯問題。


- 修復了將即時資料與多執行緒 recalc 功能結合使用時出現的問題。


- 修正當使用特定 Windows 安全性漏洞防護設定(SimExec、CallerCheck) 時，Excel 無法正常啟動或當機的問題


- 修正使用「複製格式」時，在某些情況中 Excel 可能會意外關閉的問題。


- 我們已修正使用者無法修改樞紐分析表篩選的問題，因為它已設定為Analysis Services數據庫中不再存在的值。


- 已修正當凍結工作表頂端列之後，Excel 可能會在使用 [快速分析] 時意外關閉的問題。
...
- 已修正在某些情況下，按一下同一個活頁簿中某個位置的超連結會導致活頁簿被隱藏的問題。
...
- 已修正在複製含有樞紐分析表的工作表後，會於某些情況下導致 Excel 意外關閉的問題。


- 已修正當檔案路徑太長時，在檔案重新開啟之後外部連結會停止運作的問題。


- 已修正將即時資料與多執行緒重新計算功能結合使用時，可能會導致應用程式意外關閉的問題。
...
- 我們已修正 [拼寫檢查] 功能表未載入的問題。


### <a name="outlook"></a>Outlook

- 我們已修正選用連線體驗導致網頁增益集無法載入的問題。  請參閱此處的更多詳細資訊： https://developer.microsoft.com/en-us/office/blogs/outlook-add-ins-and-optional-connected-experiences/
...
- 已修正當與雲端附件互動時，導致使用者偶爾關閉的問題。


- 解決會導致 CLP 使用者在回覆時的寄件者地址，從受保護的內容切換到未受保護的內容時會發生問題的問題。
...
- 解決會導致使用者在某些情況下遇到間歇性懸置和關閉的問題。


- 解決了當您從 POP 帳戶刪除4 或多個電子郵件時，已選取 [僅下載標題] 選項時，導致使用者遇到問題的問題。
...
- 解決使用者在第三方 MAPI 應用程式中遇到問題的問題。


- 已解決在 Windows 更新後，開啟儲存在本機的 msg 或 .oft 檔案會導致 Outlook 意外關閉的問題。


- 已解決導致 Outlook 在某些 Windows 組建上意外關閉的問題。
...
- 我們已修正導致使用者在線上模式下在資料夾之間移動多個郵件項時效能下降的問題。
...
- 我們已修正導致在傳送受 Azure 保護的語音信箱時用戶名顯示為電話號碼，從而導致傳統型 Outlook 使用者無法開啟來自外部使用者的語音信箱。


- 我們已修正一個問題，即設定 OME 設定時在郵件項目上新增無關的附件，這會迫使 Outlook 加密郵件，即使在服務端設定了DecryptAttachmentsFontryptOnly 選項。


- 我們已修正導致在某些情况下，儘管原則停用了 [僅加密] 和 [不可轉寄] 選項，但仍繼續啟用的問題。


- 我們已修正導致 SmartLinks 在儲存至草稿時丟失其格式的問題。
...
- 解決會導致使用者在擷取角色資訊時有時候遇到意外關閉的問題。


- 這可修正編輯收件者時，會導致使用者偶爾遇到應用程式關閉的問題。
...
- 已解決在從系統管理人通知中提交意見反應時，使用者遇到突然關閉的問題。
...
- 已修正使用者在同一檔案中同時有新版和舊版批註，因此觸發批註升級的問題。


- 我們已修正可能會導致應用程式意外關閉的建議窗格之問題。


- 我們修復了 [動作設定] 對話方塊中導致 PowerPoint 應用程式意外關閉的問題。


- 我們已修正 PowerPoint 應用程式可能導致其意外關閉的問題。


- 我們已修正導致「歡迎回來」的問題！ 「從先前離開的地方開始」在 PowerPoint 裏不起作用。
...
- 已修正如果資源中定義了多個成本費率表，其餘成本並不一定都能正確地計算的問題。
...
- 已修正在表單檢視中編輯前置/後續任務資料時，會觸發額外的 ProjectBeforeTaskChange 事件的問題。
...
- 已修正使用者的 TLS-DSK 憑證不會在預期時間續訂，而只在有效期剩餘不到 12 小時才續訂的問題。


- 已修正當 Office 尚未獲得授權時，商務用 Skype UI 在登入後顯示為空白的問題。


### <a name="visio"></a>Visio

- 我們已修正導致即時預覽在文字對齊時發生意外關閉的問題。
...
- 解決會導致使用者在回覆或撰寫新電子郵件時遇到問題的問題。


- 解決使用滑鼠上的「X」按鈕時，導致使用者偶爾遇到問題的問題。


- 解決會導致使用者在開啟某些非常大型電子郵件時會遇到問題的問題。


- 我們已修正貼上編號標題顯示額外縮排的問題。
...
- 已修正帶有註解副檔名的 bug，其中註解回覆的副檔名與父註解的副檔名相同。


- 我們已修正問題：如果您回復的父級註解在副檔名清單中有未知的副檔名，那麼回復將得到相同的副檔名。


- 解決了可能導致應用程式在啟動時意外關閉的問題。


- 我們已修正 Outlook 有關將郵件設定為 [不可轉寄] 的問題。
...
- 已修正舊版非網頁服務的 [共用] 窗格問題：在開啟 [共用] 窗格時關閉文件，可能會導致應用程式意外關閉。


- 已修正關閉 Office 檔案時可能導致應用程式意外關閉的計時問題。
...
- 當您啟用增益集，而登錄金鑰 HKEY_CURRENT_USER \Software\Microsoft\Internet Explorer\Main\TabProcGrowth 被設為零時，辦公主機視窗會意外關閉。 此變更將修正此問題。
...
- 修正含有批註副檔名的 bug，其中批註回復的副檔名與父批註相同。
...
- 已修正載入活頁簿之後特定函數可能會發生錯誤結果的問題。


- 我們已修正應用程式意外終止 (與 XLAM 增益集參照和命名範圍相關的問題) 的問題。


- 已修正在執行進階篩選巨集時，不正確報告錯誤的問題。
...
- 我們已修正導致使用者無法以「全域通訊清單」中隱藏的「通訊群組」的名義或以代表其傳送的問題。
...
- 已修正無法將以 [唯讀建議] 方式開啟的檔案切換至編輯功能的問題。
...
- 已修正在執行數天後，會無法啟動 32位元 Skype 商務版客戶螢幕共享的問題。

### <a name="office-suite"></a>Office 套件

- 已修正如果 [自動儲存] 已透過組織政策關閉的話，某些文件可能不會在開啟時顯示最新的伺服器內容，除非使用者點擊 [可用的更新] 才會顯現。
...
- 已修正嘗試共用儲存在 SharePoint 2016 上的檔案時，導致使用者遇到「很抱歉，發生問題，無法共用」訊息的問題。
...
- 已修正 Win7 的效能問題，其中，於所有應用程式中從功能區插入圖案庫時，大約需要 4 秒鐘的時間才會顯示。

- 已修正 Backstage 的 [資訊位置] 平板未顯示協助工具資訊的錯誤。
...
- 已修正 Office 更新訊息不以預期語言顯示的問題。 自此以後，Office 更新訊息會正確符合 Windows 顯示語言。
...
- 已修正應用程式可能在關機時懸置的各種問題。 同時修正特定相關增益集當機的問題。
