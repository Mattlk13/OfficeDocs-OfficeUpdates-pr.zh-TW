...
## <a name="version-2108-july-27"></a>版本 2108: 7 月 27 日
*版本 2108 (組建 14326.21080)*
...
- **開啟共用行事曆改良功能：** 針對 Office 365 中的共用行事曆，Outlook 可以使用 REST API 更新這些行事曆。 開啟預覽，以更快速且更可靠地更新共用行事曆。 在[深入了解](https://support.microsoft.com/en-us/office/outlook-calendar-sharing-updates-c3aec5d3-55ce-4cea-84b0-80aab6d8dc26)中查看詳細資料<br />在[部落格文章](https://insider.office.com/en-us/blog/shared-calendars-improvements-in-outlook-for-windows)中查看詳細資料

### <a name="powerpoint"></a>PowerPoint

- **Word/Excel/PowerPoint 中的 DLP 原則：** 現在可以透過應用程式偵測到其他已配置為 OneDrive 和 SharePoint 資料外洩防護 (DLP) 原則的敏感性資訊類型，以顯示原則提示。 此更新也會提供準確性改善和全球化支援。 [深入瞭解] (/microsoft-365/compliance/sensitive-information-type-learn-about) 
...
- 在某些客戶設定中，我們發現文件從 Office 匯出為 PDF 或 XPS 格式可能會因為最近的更新而停止運作。 更新修正了此問題，並重新啟用了匯出為這些格式的功能。
...
## <a name="version-2108-july-12"></a>版本 2108：7 月 12 日
*版本 2108 (組建 14326.21062)*
...
## <a name="version-2102-july-12"></a>版本 2102：7 月 12 日
*版本 2102 (組建 13801.21528)*
...
- 我們已修正可能導致使用 Access 資料庫引擎 ODBC API 的應用程式意外關閉的問題。


- 我們已修正可能導致使用 Access 資料庫引擎 OLEDB API 的應用程式搭配的資料庫包含 SharePoint 清單連結時，會意外關閉的問題。
...
- 我們已修正復原主要版本組建可能會導致應用程式在開啟檔案時意外終止的問題。
...
- 我們已修正問題，讓名稱管理員可以在具有大量隱藏名稱的書籍上開啟。


- 我們已修正在儲存至 SharePoint Online (SPO) 文件庫時，儲存的活頁簿會顯示在最近清單頂端的問題。


- 我們已修正一個問題，在開啟 CSV 檔案時，將可識別當地語系化的函數名稱。


- 我們已修正從包含 XML Map 的活頁簿中另存為 XML 資料 (.xml) 的問題。
...
- 我們已解決 Excel 在少數情況下，可能會在計算活頁簿時停止回應的問題。
...
- 我們已修正主要影響使用非 MSI 版 Office 的客戶的問題，其中 Excel 清除 HKEY_CURRENT_USER\SOFTWARE\Microsoft\Office\Common\UserInfo\Company 的值。


- 我們已修正查詢更新導致 Excel 停止回應的問題。


- 我們已修正如果 Application.DisplayAlerts 設定為 True，就無法從 VBA 開啟受 DRM 保護的活頁簿的問題。
...
- 我們已修正啟用 MAPI 的應用程式在具有 ARM 處理器的電腦上使用 Outlook 元件的問題。 此問題可能導致搜尋無法運作，或因為背景應用程式重複重新啟動而造成電腦上額外的負載。
...
- 我們已修正導致使用者無法從受保護的語音信箱訊息儲存 MP3 附件的問題。
...
- 我們已修正會導致使用者能夠下載受保護的語音信箱訊息的問題。


- 我們已修正在 Outlook 中建立會議時且 Exchange 系統管理員未設定選項時，預設出現有關縮短會議之非預期資訊型通知提示的問題。


- 我們已修正以 ICAL 轉送一些大型或持續很久的週期性會議時所發生的問題。


- 我們已修正在代表另一個使用者進行傳送，並且針對非全域通訊清單的通訊錄進行解析時，造成名稱解析無法運作的問題。
...
- 我們已修正會議項目取消表單會出現，而不是預期的編輯表單的問題。
...
- 我們已修正在 [另存新檔] 和 [儲存] 到非雲端位置期間，VML 的 ShapeID 和其他屬性寫入的問題。


- 我們已修正使用 Ctrl + 捲動放大或縮小選取的投影片放映物件時，投影片放映按鈕似乎已停用的問題。
...
- 我們已修正狀態管理員未在報告表格中更新的問題。 使用者現在應該會看到狀態管理員中反映的變更。


- 我們已修正重新排定工作的問題。 手動排程的任務排程時間可能早於 Project 中應排程的任務。
...
- 我們已修正 Word 畫布旁的註解卡片大小不正確的問題。


- 我們已修正復原主要版本組建可能會導致應用程式在開啟檔案時意外終止的問題。


- 我們已修正共同撰寫文件時，若註解順序變更，則不會清除作用中草稿的問題。


- 我們已修正顯示新視覺重新整理的對話方塊包含螢幕閱讀程式無法讀取的文字的問題。
...
- 我們已修正由於使用者登出或重新啟動電腦而電腦關機時，可能導致 Word 意外關閉的問題。
...
- 我們已修正拼字及文法檢查用畫布內容卡顯示圖示按鈕，但那些按鈕沒有工具提示的問題。
...
- 我們已修正應用程式意外關閉 .doc 檔案的問題。


- 我們已修正了當文字不應該在文字周圍顯示邊框的問題。
...
- 我們已修正在受密碼保護的 DOCX 檔案上維護引文的「最新清單」的問題。
...
- 我們已修正使用者無法編輯儲存在內部部署 SharePoint 伺服器中的特定文件的問題。


- 我們已修正開啟 SyncBacked 檔案時出現效能迴歸的問題。


- 我們已修正重新開啟特定檔案時意外關閉的問題。


- 我們已修正會導致雲端文件無法開啟的問題。


- 我們已修正在 ARM64 裝置上使用 Office 的客戶間歇性地看到不正確呈現文字的問題。


- 我們已修正 Outlook 中的自訂 VSTO 控制項在開啟和切換多個視窗和視圖之後停止運作的問題。


- 我們已修正 Office 在嘗試顯示促銷對話方塊時意外關閉的問題。
...
- 我們已修正 Office 應用程式報告可靠性問題的問題。



[//]: # (DO NOT REMOVE BUGDETAILS CONTENT END)

## <a name="version-2102-january-11"></a>版本 2102: 1 月 11 日
...
## <a name="version-2102-october-12"></a>版本 2102: 10 月 12 日
*版本 2102 (組建 13801.21004)*
...
- 已修正 Word 有時會在文字周圍顯示不應該出現的框線的問題。


- 已修正當特殊字型旋轉 90 度時，特殊字型的字元間距增加問題。
...
## <a name="version-2008-october-12"></a>版本 2008: 10 月 12 日
*版本 2008 (組建 13127.21792)*
...
- 我們已修正在 Word 中更新欄位時的問題。



[//]: # (請勿移除 BUGDETAILS 內容結尾)

## <a name="version-2008-august-10"></a>版本 2008: 8 月 10 日
*版本 2008 (組建 13127.21736)*
...
## <a name="version-2002-august-17"></a>版本 2002：8 月 17 日
*版本 2002 (組建 12527.22021)*

安全性更新列於[此處](microsoft365-apps-security-updates.md)


## <a name="version-2002-august-10"></a>版本 2002: 8 月 10 日
*版本 2002 (組建 12527.22017)*
...
- 我們已修正破壞了一些舊的 Excel 4.0 和 Excel 5.0 巨集，以及一些對 dialogsheets.show 的 VBA 呼叫問題。


- 已修正 Excel 的部分自動化增益集無法載入的問題。
...
- 我們已修正造成部分使用者的 [狀態列] 無法顯示 [準備就緒] 狀態的問題。
...
- 已修正當使用特定 Windows 安全性漏洞防護設定(SimExec、CallerCheck) 時，Excel 無法正常啟動或關閉的問題。


- 我們已透過將可在屬性 [自動完成] 下拉清單中顯示的屬性數量增加到 256 個屬性，以解決問題。
...
- 已解決在新增或儲存附件時，會導致 Outlook 偶爾停止運作的問題。
...
- 我們修正了導致在其設定檔中具有大型階層的共用或委派信箱的使用者遇到無回應的問題。
...
- 已解決導致某些使用者觀察到在離線狀態下意外啟動 Outlook 的問題。
...
- 我們已修正會導致使用者在 Outlook 重新啟動之前，在功能窗格中停用新增行事曆的問題。


- 我們已修正會導致某些會議的原始出席者在另一個出席者轉寄會議時收到取消通知的問題。


- 我們已修正會導致某些客戶在載入行事歷時遇到無回應的問題。
...
- 已修正在其他版本 Windows 中使用大聲朗讀功能時導致 Outlook 意外關閉的問題。
...
- 我們已修正會導致雲端設定使用者在更新設定時遇到意外關閉的問題。
...
- 我們新增了一個 RegKey，可讓客戶停用 IDataObject 操作 (例如，拖曳拖放、剪貼簿) 中附件的 FileTime 包含。  
    登錄機碼：
...
- 我們已修正某些使用者無法將 Forms 擴充新增至 PowerPoint 簡報的問題。


- 我們已修正儲存區更新為版本 1.0.0.2 以支援集中部署的問題。 使用者必須更新 PowerPoint 中的版本資訊，才能存取儲存區。


- 已修正使用 '保留來源格式設定' 選項複製及貼上投影片時，有時會將此設定意外地複製到新的投影片母片的問題。
...
- 已修正插入線上圖片時，應用程式無回應的問題。


- 已修正對具有 3D 效果的圖示和 SVG 圖形套用色彩的問題。
...
- 已修正 Wordmail 中當連結的第 2084 個字元是逸出字元時，某人無法傳送此項目的問題。
...
- 已修正當離線開啟 SyncBacked 檔案時，無法重新命名檔案的問題。


- 已修正如果回復到先前的組建，Word、PowerPoint 和 Excel 無法開啟雲端文件的問題。


- 已修正將註解中的文字複製/貼上到 Excel 時發生錯誤的問題。


- 已修正針對使用者停用 GCC 聽寫的問題。
...
- 已修正即使 GPO 已設為停用要求，Office 範本也會開啟的問題。


[//]: # (DO NOT REMOVE BUGDETAILS CONTENT END)

## <a name="version-2008-july-13"></a>版本 2008：7 月 13 日
*版本 2008 (組建 13127.21704)*
...
## <a name="version-2002-july-13"></a>版本 2002：7 月 13 日
*版本 2002 (組建 12527.21986)*
