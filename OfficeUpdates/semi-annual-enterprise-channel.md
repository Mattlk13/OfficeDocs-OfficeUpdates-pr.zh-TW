...
- 我們已修正會導致某些使用者在同步其信箱時遇到意外關閉的問題。


### <a name="powerpoint"></a>PowerPoint

- 我們已修正會導致使用者在顯示人員卡片並瀏覽至其他應用程式之後，看到 Office 應用程式變得沒有回應的問題。


### <a name="word"></a>Word

- 修正在儲存 Word 文件並重新開啟時，內建的敏感度標籤按鈕會呈現為灰色的問題。
...
- 修正在商務用 Skype 訊息中複製/貼上時，會導致出現對話方塊顯示「貼上您的內容時發生問題」的問題。
...
- 修正問題；您應該可以呼叫日期/時間延伸資料類型至您的程式碼，而不會在您的應用程式中遇到任何問題。
...
- 修正當凍結工作表頂端列之後，Excel 可能會在使用 [快速分析] 時意外關閉的問題。
...
- 修正在複製含有樞紐分析表的工作表後，會於某些情況下導致 Excel 意外關閉的問題。


- 修正當檔案路徑太長時，在檔案重新開啟之後外部連結會停止運作的問題。


- 修正將即時資料與多執行緒重新計算功能結合使用時，可能會導致應用程式意外關閉的問題。
...
- 我們已修正會導致在某些情况下，儘管原則停用了 [僅加密] 和 [不可轉寄] 選項，但仍繼續啟用的問題。
...
- 修正使用者在同一檔案中同時有新版和舊版批註，因此觸發批註升級的問題。
...
- 修正帶有註解副檔名的 bug，其中註解回覆的副檔名與父註解的副檔名相同。
...
- 修正舊版非網頁服務的 [共用] 窗格問題：在開啟 [共用] 窗格時關閉文件，可能會導致應用程式意外關閉。


- 修正關閉 Office 檔案時可能導致應用程式意外關閉的計時問題。
...
- 修正使用者在使用 VBA 巨集清除範圍內容時可能遇到的效能問題。
